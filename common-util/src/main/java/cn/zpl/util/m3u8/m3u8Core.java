...
//        download(m3u8);
    ...
    public void domain() {
        ...
        CommonIOUtils.binPath(file, m3u8List, "^\\d+.m3u8");
        m3u8List.forEach(path -> {
            try {
                downloadByM3U8(path);
    ...
    public void downloadByM3U8(String path) throws IOException, InterruptedException {
        //设置好初始路径
        folderpath += File.separator + foldername;
        ...
        System.out.println("时长: " + ((int) duration / 60) + "分" + (int) duration % 60 + "秒");
        //重命名文件名
        m3u8.getTsList().stream().forEach(m3U8Ts -> {
            int start = m3U8Ts.getFile().lastIndexOf("tipsid") + 6;
            String newName = m3U8Ts.getFile().substring(start, m3U8Ts.getSubUrl().indexOf(".ts", start));
            newName = (Integer.parseInt(newName) + 1) + ".ts";
            m3U8Ts.setFile(newName);
        });
        //根据M3U8对象下载视频段
        download(m3u8);
...
//            writer.close();
    ...
    public String buildM3U8File(M3U8 m3u8) {
        ...
        File m3u8File = new File(m3u8.getFpath(), fileName + ".m3u8");
